...
from ..database import get_db, User, ActiveSyncDevice, ActiveSyncState, CalendarEvent
...
async def eas_options(request: Request):
    ...
    _write_json_line("activesync/activesync.log", {"event": "options",
...
def _get_or_create_device(db: Session, user_id: int, device_id: str, device_type: str | None = None) -> ActiveSyncDevice:
    device = db.query(ActiveSyncDevice).filter(
        ActiveSyncDevice.user_id == user_id,
        ActiveSyncDevice.device_id == device_id
    ).first()
    if not device:
        device = ActiveSyncDevice(user_id=user_id, device_id=device_id, device_type=device_type or "generic")
        db.add(device)
        db.commit()
        db.refresh(device)
    return device

def _get_or_init_state(db: Session, user_id: int, device_id: str, collection_id: str = "1") -> ActiveSyncState:
    ...
    if not state:
        state = ActiveSyncState(user_id=user_id, device_id=device_id, collection_id=collection_id, sync_key="1")
        db.add(state)
        db.commit()
        db.refresh(state)
    return state
...
async def eas_dispatch(request: Request, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    ...
    if cmd == "sync":
        ...
        _write_json_line("activesync/activesync.log", {"event": "sync_response", "bytes": len(xml_response)})
        return Response(content=xml_response, media_type="application/vnd.ms-sync.wbxml", headers=headers)
    if cmd == "ping":
    ...
    if cmd == "getitemestimate":
        ...
        _write_json_line("activesync/activesync.log", {"event": "getitemestimate", "collection": collection_id})
    ...
    if cmd == "foldersync":
        ...
        xml = f"<FolderSync><Status>1</Status><SyncKey>{state.sync_key}</SyncKey><Changes>" \
              f"<Add><ServerId>{collection_id}</ServerId><ParentId>0</ParentId><DisplayName>Inbox</DisplayName><Type>2</Type></Add>" \
              "</Changes></FolderSync>"
        _write_json_line("activesync/activesync.log", {"event": "foldersync", "sync_key": state.sync_key})
        return Response(content=xml, media_type="application/vnd.ms-sync.wbxml", headers=headers)
    if cmd == "settings":
    ...
    if cmd == "search":
        ...
        try:
            ...
            if not query and body:
                ...
                if "<Query>" in txt:
                    qstart = txt.find("<Query>") + 7; qend = txt.find("</Query>")
        ...
        users = db.query(User).filter(
            (User.email.ilike(q)) | (User.username.ilike(q)) | (User.full_name.ilike(q))
        ).order_by(User.full_name.asc()).limit(50).all()
        # Build EAS Search response for GAL
        root = ET.Element("Search"); ET.SubElement(root, "Status").text = "1"
        resp = ET.SubElement(root, "Response"); store = ET.SubElement(resp, "Store"); ET.SubElement(store, "Name").text = "GAL"
        for u in users:
            result = ET.SubElement(store, "Result"); props = ET.SubElement(result, "Properties")
            ET.SubElement(props, "DisplayName").text = u.full_name or u.username
            ET.SubElement(props, "EmailAddress").text = u.email
            ET.SubElement(props, "FirstName").text = (u.full_name or u.username).split(" ")[0]
            last = (u.full_name or u.username).split(" ")[-1]
            ET.SubElement(props, "LastName").text = last if last else (u.full_name or u.username)
...
def _calendar_to_eas_xml(events: list[CalendarEvent]) -> str:
    root = ET.Element("Sync"); root.set("xmlns", "AirSync")
    col = ET.SubElement(root, "Collection"); col.set("Class", "Calendar"); col.set("SyncKey", "1"); col.set("CollectionId", "calendar")
    for ev in events:
        add = ET.SubElement(col, "Add"); add.set("ServerId", str(ev.id))
        ...
        ET.SubElement(data, "StartTime").text = ev.start_time.strftime("%Y-%m-%dT%H:%M:%SZ")
...
@router.get("/calendar")
def calendar_list(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    events = db.query(CalendarEvent).filter(CalendarEvent.user_id == current_user.id).order_by(CalendarEvent.start_time.asc()).limit(200).all()
    return [{
        "id": e.id,
        "title": e.title,
...
def calendar_create(request: Request, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    import json
    data = json.loads(request._body.decode('utf-8')) if hasattr(request, '_body') and request._body else {}
    ...
    event = CalendarEvent(user_id=current_user.id, title=title, description=data.get('description'), location=data.get('location'), start_time=start, end_time=end, is_all_day=bool(data.get('is_all_day')))
    db.add(event); db.commit(); db.refresh(event)
    return {"id": event.id}

@router.get("/calendar/sync")
def calendar_sync(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    events = db.query(CalendarEvent).filter(CalendarEvent.user_id == current_user.id).order_by(CalendarEvent.start_time.asc()).limit(200).all()
    xml = _calendar_to_eas_xml(events)
    return Response(content=xml, media_type="application/vnd.ms-sync.wbxml", headers=_eas_headers())

# Root-level aliases (some clients call without /activesync prefix)
@router.options("/../Microsoft-Server-ActiveSync")
async def eas_options_alias(request: Request):
    headers = _eas_headers()
    _write_json_line("activesync/activesync.log", {"event": "options_alias",
        "ip": (request.client.host if request.client else None),
        "ua": request.headers.get("User-Agent"),
        "host": request.headers.get("Host"),
    })
...
@router.post("/../Microsoft-Server-ActiveSync")
async def eas_dispatch_alias(request: Request, current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
