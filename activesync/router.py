...
# Root-level alias endpoints removed. Root-level routes are now registered in app/main.py
...
<FolderSync xmlns="FolderHierarchy">
    <Status>8</Status>
        ...
        # iOS fetch requests expect full MIME bodies even if BodyPreference was omitted.
        if body_type_preference != 4 and fetch_ids:
            body_type_preference = 4

        if body_type_preference == 4:
            effective_truncation = truncation_size  # None means no truncation for MIME
        else:
    ...
    <Collections>
        ...
        if client_sync_key == "0":
            ...
            if is_wbxml_request:
                ...
                # Build payloads with both HTML and plain fallbacks when available
